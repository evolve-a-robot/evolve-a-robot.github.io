...
var D3_Barchart = D3_Barchart || new function() {
  ...
  var Generic_Barchart = function() {
    ...
    this.updateBarchart = function(new_data) {
      for (var i = 0; i < new_data.length; ++i) {
        fitness_data.push(new_data[i][1] > 0 ? new_data[i] : [new_data[i][0],0.1,new_data[i][2]]);
      }
    ...
    this.updatePopSize = function(new_pop_size) {
      ...
      col_width = (width-2*padding) / pop_size;
  ...
  this.clearPopulationBarchart = function() {
    pop_barchart.clearData();
  }

  // Update the population size.
  this.updatePopSize = function(new_pop_size) {
    pop_barchart.updatePopSize(new_pop_size);
  }
};