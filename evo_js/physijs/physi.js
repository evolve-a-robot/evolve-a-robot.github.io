window.Physijs = (function() {
	...
	Physijs.Scene = function( params ) {
		...
		params = params || {};
		params.ammo = "../ammo.js";//Physijs.scripts.ammo || 'ammo.js';
	...
	Physijs.Scene.prototype.replaceWorker = function () {
        if (!this._worker) {
            return;
        }
        var newWorker = new Worker(Physijs.scripts.worker);
        newWorker.transferableMessage = this._worker.transferableMessage;
        newWorker.onmessage = this._worker.onmessage;
        var ab = new ArrayBuffer(1);
        this._worker.transferableMessage(ab, [ab]);
